...
function valeriyGrachunez(question) {
  ...
  for (let kw of dandyKeywords) {
    if (q.includes(kw)) return "🍑 ХРЕНЬ ЭТО! Валерий говорит — полная хрень! 💀";
  }
  const brainrotKeywords = ['brainrot', 'skibidi', 'сигма'];
  for (let kw of brainrotKeywords) {
    if (q.includes(kw)) return "🔥 ЭТО КРУТО! Валерий Грачунез одобряет! 🧠💀";
  }
  if (q.includes('?')) {
    const yes = ["ДА, КОНЕЧНО! 🤪", "НУ ТИПА ДА, БАЗА! 💪"];
    const no = ["НЕТ, ТОЧНО НЕТ! 🚫", "НЕ, НУ ТЫ ЧЁ? НЕТ! 🤡"];
    return Math.random() > 0.3 ? yes[Math.floor(Math.random() * yes.length)] : no[Math.floor(Math.random() * no.length)];
  }
  const folk = ["🍕 НУ ТИПА ЭТО БАЗА", "🧠 ВАЛЕРИЙ ЗА ПИВО! 🍺", "🤡 КРИНЖ или БАЗА? А похуй!"];
...
app.post('/register', (req, res) => {
  ...
  users.set(username, { password, friends: [], avatar: null, roles: ['Пользователь'], registeredAt: new Date() });
...
app.post('/login', (req, res) => {
  ...
  if (user && user.password === password) {
    res.json({ success: true, friends: user.friends || [], avatar: user.avatar, roles: user.roles });
  } else {
...
app.get('/globalMessages', (req, res) => res.json(globalMessages.slice(-100)));

app.post('/globalMessages', (req, res) => {
  globalMessages.push(req.body);
  res.json({ success: true });
});

app.post('/editMessage', (req, res) => {
  const { messageId, newText, chatType, chatId } = req.body;
  let messagesArray = chatType === 'global' ? globalMessages : privateMessages.get(chatId) || [];
  const msg = messagesArray.find(m => m.id == messageId);
  if (msg) {
    msg.text = newText;
    msg.edited = true;
    msg.editTime = new Date();
    res.json({ success: true });
  } else {
    res.json({ success: false });
  }
...
app.post('/addReaction', (req, res) => {
  const { messageId, emoji, username, chatType, chatId } = req.body;
  let messagesArray = chatType === 'global' ? globalMessages : privateMessages.get(chatId) || [];
  const msg = messagesArray.find(m => m.id == messageId);
  if (msg) {
